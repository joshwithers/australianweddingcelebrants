...
test("editorial author fallback preserves explicit authors", () => {
  ...
  assert.deepEqual(
    resolveEditorialAuthor({ name: "  Casey Example  ", url: "https://example.com/casey" }),
...
test("all and only original editorial pages carry Frankie attribution", async () => {
  ...
  for (const article of EDITORIAL_ARTICLES) {
    ...
    const articleSchema = jsonLdFrom(html).find((item) => item["@type"] === "Article");
...
test("RSS creator and Markdown author stay in parity", async () => {
  ...
  assert.equal((rss.match(/<dc:creator>Frankie<\/dc:creator>/g) || []).length, EDITORIAL_ARTICLES.length);
  for (const article of EDITORIAL_ARTICLES) {
...
test("public pages render the exact cookieless GA4 consent boundary", async () => {
  ...
  assert.ok(consentAt >= 0 && loaderAt > consentAt, "consent defaults must precede gtag.js");
  for (const key of [
    "analytics_storage",
    "ad_storage",
    "ad_user_data",
  ...
  assert.doesNotMatch(source, /localStorage|sessionStorage|document\.cookie|consent[^\n]+update/i);
  ...
  for (const pathname of paths) {
    ...
    assert.ok(deniedAt >= 0 && tagAt > deniedAt, `${pathname} loads GA before denied defaults`);
  ...
  assert.match(submitRedirect, /api\.australianweddingcelebrants\.com\.au\/login/);
});

test("profile HTML, Markdown and JSON expose provenance without author or unsupported credentials", async () => {
  const html = await read(`../dist/directory/${PROFILE_SLUG}/index.html`);
  assert.doesNotMatch(html, /<meta name="author"/);
  assert.match(html, /data-tier-evidence="missing"/);
  ...
  assert.match(markdown, /## Tier evidence/);
...
test("LLM surfaces and privacy notice publish provenance and measurement limits", async () => {
  ...
  assert.match(privacy, /does not make a promise of anonymity or guaranteed non-identifiability/);
...
test("tier evidence currency is deterministic", () => {
  ...
  assert.equal(getTierEvidence({}, now).status, "missing");
  assert.equal(
    getTierEvidence(
      {
